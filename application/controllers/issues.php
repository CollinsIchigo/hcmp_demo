...
class issues extends MY_Controller {
	...
	public function district_store_drug_recieval()
		...
		if ($this -> input -> post('mfl')) :
			//assign values from the post to variables
			$facility_code = $this -> session -> userdata('facility_id');
			$district_id = $this -> session -> userdata('district_id');
			$service_point = array_values($this -> input -> post('mfl'));
			$commodity_id = array_values($this -> input -> post('desc'));
			...
			//array to hold values from the post
			$data_array_issues_table = array();
			$data_array_redistribution_table = array();
			$new_drug_store_entry = array();
			
			$digested_expiry = $expiry_date[0];
			//lop through the data posted
			for ($i = 0; $i < $total_items; $i++) :
				
				//convert stock value into units
				$total_items_issues = ($commodity_unit_of_issue[$i] == 'Pack_Size') ? $quantity_issued[$i] * $total_units[$i] : $quantity_issued[$i];

				//prepare the issues data
				$facility_name = isset($service_point[$i]) ? Facilities::get_facility_name2($service_point[$i]) : null;
				//echo "<pre>";print_r($service_point[$i]);exit;
				$facility_name = isset($facility_name) ? $facility_name['facility_name'] : 'District Store';
				
				//holds details about the redistribution ie from which point to which point
				$mydata = array('facility_code' => $facility_code, 
								's11_No' => '(-ve Adj) Stock Deduction', 
								'batch_no' => $batch_no[$i], 
								'commodity_id' => $commodity_id[$i], 
								'expiry_date' => date('y-m-d', strtotime($expiry_date[$i])), 
								'qty_issued' => $total_items_issues, 
								'issued_to' => "inter-facility donation:" . $facility_name, 
								'balance_as_of' => $commodity_balance_before[$i], 
								'date_issued' => date('y-m-d', strtotime($clone_datepicker_normal_limit_today[$i])), 
								'issued_by' => $this -> session -> userdata('user_id'));
				
				//holds the specific details about the redistribution ie commodity details - amounts involved etc
				$mydata_2 = array('manufacturer' => $manufacture[$i],
								 ...
								 'commodity_id' => $commodity_id[$i], 
								 'expiry_date' => date('y-m-d', strtotime($expiry_date[$i])), 
								 'quantity_sent' => $total_items_issues, 
								 'receive_facility_code' => $service_point[$i], 
								 'facility_stock_ref_id' => $facility_stock_id[$i], 
								 'date_sent' => date('y-m-d'), 
								 'sender_id' => $this -> session -> userdata('user_id'));
								 
				
				// push the two arrays into the two sets of arrays
				array_push($data_array_issues_table, $mydata);
				array_push($data_array_redistribution_table, $mydata_2);
				// reduce the stock levels
				$a = Doctrine_Manager::getInstance() -> getCurrentConnection();
				$a -> execute("UPDATE `facility_stocks` 
								SET 
								    `current_balance` = `current_balance` - $total_items_issues
								WHERE
								    id = '$facility_stock_id[$i]'");
									
				//update the transaction table here
				$inserttransaction = Doctrine_Manager::getInstance() -> getCurrentConnection();
				$inserttransaction -> execute("UPDATE `facility_transaction_table` 
												SET 
												    `total_issues` = `total_issues` + $total_items_issues,
												    `closing_stock` = `closing_stock` - $total_items_issues
												WHERE
												    `commodity_id` = '$commodity_id[$i]'
												        AND status = '1'
				...
				if ($existence[0]['present'] >= 0) {
					$update_totals = Doctrine_Manager::getInstance() -> getCurrentConnection();
					$update_totals -> execute("UPDATE `drug_store_totals` 
												SET 
												    `total_balance` = `total_balance` + $total_items_issues
												WHERE
												    `commodity_id` = '$commodity_id[$i]'
